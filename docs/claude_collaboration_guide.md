...
- **무선**: `rc-auto-pattern` 모드가 별도 펌웨어(rc-mix-ppm 수동 프로파일 +
  ...
  (rc_auto_pattern_preview.png)를 out-dir에 생성. 텔레메트리는 USB 호스트가
  읽을 때만 출력(무선에서 CDC 블로킹으로 루프가 밀리는 것 방지 — 2026-06-12
  "선 빼면 수동이 밀림" 증상의 원인). 피벗은 1.5초간 회전 5° 미만이면 B를
  30%씩 증폭(상한 0.6) — 정지마찰/배터리 강하 자동 보상. 온보드 위치는
