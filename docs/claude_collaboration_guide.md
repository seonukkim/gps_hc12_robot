...
- **무선**: `rc-auto-pattern` 모드가 별도 펌웨어(full-telemetry-ppm 수동
  프로파일 + IMU + 온보드 ㄹ 패턴)를 업로드한다. CH5 MANUAL=스틱 주행,
  MANUAL 1초 후 AUTO 플립=패턴 1회(타이머 레인 + IMU 피벗), MANUAL 복귀=즉시
  정지. 부팅 시 AUTO면 절대 안 움직임(재무장 필요). 패턴 파라미터는 업로드 시
  컴파일로 구워짐(--lane-ms 등). 업로드 시 예상 경로 PNG
  (rc_auto_pattern_preview.png)를 out-dir에 생성. 온보드 위치는
  IMU+시간 추측항법(GPS 1-2m 오차가 레인급이라 온보드 GPS 보정은 의도적 제외).
- **무선 헤딩 체인(2026-06-12)**: AUTO 시작 순간의 yaw를 기준(RUN_START)으로
  모든 스텝이 **절대 바디 헤딩 목표**를 가진다. 직진은 heading-hold
  (`B = drive_steer_sign × clamp(오차×--heading-kp, ±--heading-hold-max-b)
  + --drive-b-trim`, 전·후진 동일)로 휨을 매 틱 보정하고, 피벗은 남은 각의
  부호로 매 틱 방향을 다시 골라 오버슈트도 되돌아온다(조기 정지 래치 없음).
  상대 목표가 아니므로 코너 잔차·레인 휨이 누적되지 않는다.
- **★주행 중 B 반응 반전(필드 실증 2026-06-12, 절대 잊지 말 것)**: 이 구동계는
  **정지 피벗에서는 +B→+yaw, 주행 중에는 반대(−c·B)**다(전·후진 동일).
  로그 증거: 피벗 2회 수렴(-39.6→+2.5, -50.6→-0.6) vs 주행 3구간 모두 같은
  부호로 발산(-0.3→-148.9 폭주, B 포화 0.25, 모터 0.55 포화). 그래서
  `--drive-steer-sign` 기본 -1 (레인 피드백에만 적용, 피벗 불변). 직진 중
  `heading_err_deg`가 줄어야 정상 — 커지면 부호를 +1로 재업로드해 비교.
  레인 오차가 `--drive-abort-err-deg`(60°)를 넘으면 도넛 방지를 위해 그
  레인을 조기 종료(`DRIVE_ABORT_HEADING` 출력).
- **회전을 진짜 90°로 만드는 3중 장치(2026-06-12)**: ① 관성 보상 조기 정지
  (`--turn-coast-s` 0.15: 정지 명령을 |yaw rate|×0.15s 만큼 일찍) ② 정지·정착
  후 재측정—허용오차 밖이면 같은 피벗 재실행(`--turn-settle-retries` 2,
  `TURN_SETTLE_RETRY` 출력) ③ 정지 시 자이로 바이어스 자동 재영점(아래).
- **자이로 온도 드리프트 자동 보상(2026-06-12)**: 부팅 캘리브레이션 바이어스가
  발열로 이동(-1.3°/s 정지 드리프트 실측). 0.7초 이상 정지가 지속되면
  바이어스를 현재 원시값으로 천천히(τ≈2s) 재추정. 스텝 사이 일시정지 기본
  800ms(--pause-ms)로 매 스텝 경계가 재영점 기회가 된다. "처음엔 잘 되다
  시간 지나면 틀어진다"의 해답.
- **무선 재시작 규칙**: MANUAL→AUTO 플립마다 스텝을 재계산하고 현재 자세를
  새 원점으로 **무조건 0번 스텝부터** 시작한다(이전 부분 실행 재개 없음).
- **무선 RC 유실 유예**: 신호 유실 시 모터는 즉시 정지(failsafe)하되,
  `--rc-loss-grace-ms`(기본 1500) 이내 복귀하면 진행 중이던 스텝을 이어서
  실행(타이머는 유실 시간만큼 시프트). 초과하면 리셋+무장해제 — 다시
  MANUAL(1초)→AUTO 필요(이때도 처음부터). 과거 "직진→회전→약간 직진 후
  멈춤"은 순간 RC 글리치가 런을 즉사시키던 동작이 원인 후보.
- **USB 출력 가드(2026-06-12)**: 모든 디버그 출력은 (a) 호스트 게이트를 최대
  1초에 1번만 평가(코어의 `(bool)Serial`이 호출당 ~10ms일 수 있음), (b) 출력
  소요시간을 측정해 50ms를 넘기면(모니터 방치/포트만 열린 채 리더 죽음) 전체
  출력을 5초 뮤트 후 1패킷 프로브로만 재시도. → 꽂은 상태/뺀 상태/모니터
  멈춤 상태 전부에서 수동 조작이 밀리지 않는다. USBDBG의
  `loop_dt_max_ms`(직전 주기 최악 루프 시간)가 작은데도 조작이 밀리면 원인은
  펌웨어 밖(배터리/모터·ESC 열/송신기)이다.
- **발열 저감**: `LOOP_IDLE_WFI`(기본 1)로 루프 사이 CPU를 인터럽트 대기
  절전(WFI)시킨다(루프 ≥~1kHz 유지, 추가 지연 ≤1ms). busy-spin 자체 발열
  제거 — 보드가 "많이 뜨겁다"(2026-06-12 보고) 완화책. 잔여 발열은 GPS/HC-12/
  수신기 전원 레일과 날씨 몫이므로 그늘/휴식/배터리 점검이 함께 필요.
  피벗은 1.5초간 회전 5° 미만이면 B를 30%씩 증폭(상한 0.6, 방향 반전 시
  기본값 복귀) — 정지마찰/배터리 강하 자동 보상.
...
9. **무선(rc-auto-pattern)에서 직진이 한쪽으로 휜다** → 모니터의
   `heading_err_deg`가 줄지 않으면 `--heading-kp`를 0.02~0.03으로,
   일정한 기계적 편향이면 `--drive-b-trim`(좌로 휘면 음수)을 추가해 재업로드.
10. **무선 패턴이 몇 스텝 만에 영영 멈춘다** → RC 글리치로 grace 초과 여부
   확인(`rc_loss_grace_ms` 늘려 재업로드). 재개하려면 MANUAL(1초)→AUTO —
   이때는 의도적으로 처음부터 재시작된다.
11. **꽂아두면/모니터 띄워두면 수동이 밀린다** → USBDBG `loop_dt_max_ms`와
   `usb_print_stall_count` 확인. stall 카운트가 증가하면 호스트가 포트를 열고
   읽지 않는 상태(출력 가드가 자동 뮤트하므로 구버전 펌웨어인지부터 확인).
   `loop_dt_max_ms`가 작은데도 밀리면 펌웨어 밖: 배터리 전압, ESC/모터 온도,
   송신기 배터리 순으로 점검(전원 끄고 식힌 뒤 회복되면 열/배터리 확정 —
   2026-06-12 현장에서 과열 해소 후 수동 정상 확인됨).
12. **직진 중 heading_err_deg가 점점 커진다(폭주)** → 조향 부호 문제.
   기본 `--drive-steer-sign -1`이 이 로버의 실측값. 만약 -1에서도 발산하면
   +1로 재업로드해 비교(§4.4 ★항목). `DRIVE_ABORT_HEADING`이 자주 뜨면
   kp를 줄이거나 표면 상태(한쪽 바퀴 걸림)를 확인.
13. **회전이 90°가 안 된다** → 모니터에서 TURN 후 PAUSE의 `heading_err_deg`
   확인: tol(8°) 이내면 정상 동작이고 시각적 차이는 시작 자세 문제.
   `TURN_SETTLE_RETRY`가 매번 2회 다 소진되면 coast가 큰 것 —
   `--turn-coast-s`를 0.2~0.25로. 정지 상태에서 yaw가 흘러가면(±0.5°/s 이상)
   재영점이 작동할 정지 시간이 부족한 것 — `--pause-ms`를 1000으로.
14. **PPM 카운터 진단**: `ppm_incomplete_frames`/`ppm_short_rejects`가 운용 중
   계속 늘면 수신기 신호선 노이즈(실측: 2µs 글리치). 배선/전원 분리 점검.
   `ppm_age_ms=42949672xx`(음수 언더플로)는 구펌웨어 표시 버그 — 최신 펌웨어로.
