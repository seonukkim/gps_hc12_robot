"""station_mission_node ament_python 패키징 설정 / ament_python packaging for the ROS2 skeleton.

목적/역할:
    스테이션 미션/운영 워크플로 노드 스켈레톤 패키지를 `colcon`/`ament_python`으로 빌드·설치
    하기 위한 setuptools 설정. 이 파일 자체는 노드가 아니라 **빌드 메타데이터**이며, 실제
    노드 스켈레톤은 `station_mission_node.node`에 있다. 현재 운영 워크플로는 통합 CLI로
    이뤄지며 이 패키지는 그 경로와 무관하다.

시스템 내 위치:
    - `package.xml`(build_type: ament_python)과 짝을 이루며, `colcon build`가 이 setup을
      호출한다. `ros2 run station_mission_node station_mission_node`로 실행할 수 있게
...
EN: setuptools/ament_python build config so `colcon build` can package and install the
    station_mission_node ROS2 (Jazzy) skeleton and register its `ros2 run` entry point. This is
    build metadata, not the node itself; it is unrelated to the current unified-CLI workflow.
"""

from setuptools import setup

package_name = "station_mission_node"
